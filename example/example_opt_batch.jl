...
read_dir = joinpath(__TASOPTroot__,"../example/ModelSaved/JetFuel_NoACT_Opt/JetFuel_NoACT_Opt3000.jld2")
# Path to save the models from optimization
save_dir = joinpath(__TASOPTroot__,"../example/ModelSaved/")
save_key = "Opti_Jet" # Optimized results saved under folder save_dir/save_key/save_key*range.xxx
...
max_num_iter_opt = [20000, 800, 10000] # Maximum number of optimization steps [Global, Local Coarse, Local Fine]
...
optimization_ranges = collect(2700:100:2800) #[nmi] ranges to optimize the current aircraft !!! Will be converted to meter later !!!
mis_opt = Requirement[]
push!(mis_opt, Requirement(:(parm[imRange,1]), 300*1852.0)) #[m] Range !!! To be updated across various missions !!!
...
push!(mis_opt, Requirement(:(fuse_tank.ACT_eta_vol), 0.88))  # Volumetric efficiency of ACT fuel tank
push!(mis_opt, Requirement(:(fuse_tank.ACT_eta_wei), 0.93))  # Gravimetric efficiency of ACT fuel tank
...
push!(par_opt, Parameter(:(wing.layout.sweep), 30.0, 60.0, 0.0, 3.0)) # [deg] Wing sweep angle 
push!(par_opt, Parameter(:(wing.layout.AR), 10.0, 20.0, 5.0, 0.75)) # Wing aspect ratio
push!(par_opt, Parameter(:(wing.inboard.cross_section.thickness_to_chord), 0.2, 0.6, 0.04, 0.03)) # Wing inner thickness to chord
push!(par_opt, Parameter(:(wing.outboard.cross_section.thickness_to_chord), 0.2, 0.6, 0.04, 0.03))
push!(par_opt, Parameter(:(wing.inboard.λ), 0.7, 1.0, 0.1, 0.05)) # Wing inner taper ratio
push!(par_opt, Parameter(:(wing.outboard.λ), 0.3, 1.0, 0.1, 0.05))
push!(par_opt, Parameter(:(parg[igyeng]), fuse_radius*3.0, fuse_radius*4.0, fuse_radius*2.0, fuse_radius*0.1)) # [m] Span wise engine location
push!(par_opt, Parameter(:(wing.layout.ηs), 0.3, 0.4, 0.2, 0.01)) # Wing span break location over half span
push!(par_opt, Parameter(:(para[iarclt, ipclimb2:ipdescent4, 1]), 1.0, 2.0, 0.4, 0.08)) # Wing cl ratio of tip to root at cruise
push!(par_opt, Parameter(:(para[iaCL, ipclimb2:ipdescent4, 1]), 0.6, 1.00, 0.3, 0.04)) # Wing CL at cruise
push!(par_opt, Parameter(:(para[iaalt, ipcruise1, 1]), 10000.0, 20000.0, 4000.0, 800.0)) # [m] Cruise altitude
push!(par_opt, Parameter(:(pare[iepif, ipclimb2:ipdescent4, 1]), 2.0, 4.0, 1.25, 0.14)) # Fan PR at cruise 
push!(par_opt, Parameter(:(pare[iepihc,ipclimb2:ipdescent4, 1]), 10.0, 50.0, 1.25, 1.0)) # HPC PR at cruise
push!(par_opt, Parameter(:(pare[ieBPR, ipclimb2:ipdescent4, 1]), 8.0, 30.0, 1.0, 1.5)) # Fan BPR at cruise
push!(par_opt, Parameter(:(pare[ieTt4, ipclimb2:ipdescent4, 1]), 1500.0, 2000.0, 1000.0, 50.0)) #[K] Turbine inlet temperature  at cruise
...
save_dir_actual = joinpath(save_dir,save_key) #Actual directory to save data
...
#### Optimization
for (i,ran_cur) in enumerate(optimization_ranges)
    # Choose whether to run global
    run_global = (i==1) #only run global for the first case

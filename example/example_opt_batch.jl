...
read_ac = joinpath(__TASOPTroot__,"../example/ModelSaved/Opti_Eth_ACT_NoComp_UniEta_NoWSpan_/Opti_Eth_ACT_NoComp_UniEta_NoWSpan_1700.jld2")
read_par = joinpath(__TASOPTroot__,"../example/ModelSaved/Opti_Eth_ACT_NoComp_UniEta_NoWSpan_/Opti_Eth_ACT_NoComp_UniEta_NoWSpan_1700_optimized_parameters.csv")
# Path to save the models from optimization
save_dir = joinpath(__TASOPTroot__,"../example/ModelSaved/")
save_key = "Opti_Eth_ACT_NoComp_UniEta_NoWSpan" # Optimized results saved under folder save_dir/save_key/save_key*range.xxx
...
optimization_ranges = collect(1800:100:3000) #collect(300:100:3000) #[nmi] ranges to optimize the current aircraft !!! Will be converted to meter later !!!
mis_opt = Requirement[]
...
push!(mis_opt, Requirement(:(options.has_ACT_fuel), true))  # Whether to allow additional center fuel tank (ACT)
...
# 6) Optionally overwrite the starting parameters if skip the initial global run
if flag_skip_global
    par_opt = load_csv_parameters(read_par)

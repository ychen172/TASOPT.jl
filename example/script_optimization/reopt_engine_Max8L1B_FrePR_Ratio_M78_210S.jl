...
par_path_base = joinpath(__TASOPTroot__,"../example/ModelSaved/Opti_Jet_NoACT_V4_4_R1Sz_EtasEng_/Opti_Jet_NoACT_V4_4_R1Sz_EtasEng_2400.jld2")
# Path to save the model from optimization
save_dir = joinpath(__TASOPTroot__,"../example/ModelSaved/")
save_key = "Opti_Jet_NoACT_FixRatio_Max8_210S_M078_Glo"
...
# Setup fixed LPC and HPC stage ratio
nLPC = 3.0
nHPC = 10.0
...
push!(mis_opt, Requirement(:(pare[iepib, :, 1]), 0.9571535600320473)) #Combustor pressure ratio
push!(mis_opt, Requirement(:(pare[ieepolf, :, 1]), 0.9331575190947989)) #Fan Poly Eff
push!(mis_opt, Requirement(:(pare[ieepollc, :, 1]), 0.9136979092725429)) #LPC Poly Eff
push!(mis_opt, Requirement(:(pare[ieepolhc, :, 1]), 0.9184147262516498)) #HPC Poly Eff
push!(mis_opt, Requirement(:(pare[ieepolht, :, 1]), 0.9430534237993433)) #HPT Poly Eff
push!(mis_opt, Requirement(:(pare[ieepollt, :, 1]), 0.9366604732362671)) #LPT Poly Eff
push!(mis_opt, Requirement(:(pare[ieetab, :, 1]), 0.9861805387928644)) #Combustion Eff
push!(mis_opt, Requirement(:(parg[igTmetal]), 1318.6189617045563)) #Maximum metal temperature
...
push!(bound_glob, Parameter(:(para[iarcls, ipclimb2:ipdescent4, 1]), 1.0, 2.0, 0.4, 0.15)) # Wing cl ratio break-root   #7
push!(bound_glob, Parameter(:(para[iarclt, ipclimb2:ipdescent4, 1]), 1.0, 2.0, 0.4, 0.15)) # Wing cl ratio tip-root     #8
push!(bound_glob, Parameter(:(para[iaCL, ipclimb2:ipdescent4, 1]), 0.6, 1.00, 0.3, 0.07)) # Wing CL at cruise           #9
push!(bound_glob, Parameter(:(para[iaalt, ipcruise1, 1]), 10000.0, 20000.0, 4000.0, 1600.0)) # [m] Cruise altitude      #10
push!(bound_glob, Parameter(:(pare[iepif, ipclimb2:ipdescent4, 1]), 2.0, 4.0, 1.25, 0.2)) # Fan PR at cruise            #11
push!(bound_glob, Parameter(:(pare[iepihc,ipclimb2:ipdescent4, 1]), 10.0, 60.0, 1.25, 1.0)) # HPC PR at cruise          #12
push!(bound_glob, Parameter(:(pare[ieBPR, ipclimb2:ipdescent4, 1]), 6.0, 12.0, 1.0, 0.5)) # Fan BPR at cruise           #13
push!(bound_glob, Parameter(:(pare[ieTt4, ipclimb2:ipdescent4, 1]), 1500.0, 2000.0, 1000.0, 100.0)) #[K] Tt4 at cruise  #14
push!(bound_glob, Parameter(:(vtail.layout.AR), 2.0, 5.0, 1.0, 0.2)) #Vertical tail aspec ratio                         #15
...
#### Push requirements and optimization parameters depending on fuselage layout
...
push!(bound_glob, Parameter(:(parg[igyeng]), fuse_radius*3.0, fuse_radius*10.0, fuse_radius*1.75, fuse_radius*0.2)) # [m] Span wise engine location
...
if flag_skip_global
    for (j,para_cur) in enumerate(par_opt)
        ...
        try # Many of the variable (Need to check why cruise value were set to many phases)
            para_cur.val = getNestedProp_fromExpr(ac; parm_sym=para_cur.name)[5] #works for ipclimb2:ipdescent4 type of setup
        catch

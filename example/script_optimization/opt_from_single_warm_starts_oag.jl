...
The design-point payload and range are also optimization parameters to give the best off-design performance.
Use job array from orcd cluster to process multiple cases at the same time
The design-point mission will be the R1 mission of the aircraft
...
for i in 1:1:length(seat_cap_keys)
    ...
    ranges_off_nmi = miss_off_des[seat_cap_keys[i]].ranges_nmi #Off-design ranges (nmi)
    weights_off = miss_off_des[seat_cap_keys[i]].weights #Off-design missions' weightings (add to 1)
    ...
    save_vec_struct_csv(joinpath(save_dir_actual, "$(save_key)_$(round(Int,seat_cap_keys[i]))_mission_requirements.csv"), mis_opt_cur)
    ...
    if status_found in success_statuses
        ...
        if sized_succeeded
            quicksave_aircraft(ac_copy, joinpath(save_dir_actual, "$(save_key)_$(round(Int,seat_cap_keys[i])).jld2"))
        end
    end
    
    #### save the results
    open(status_log, "a") do io
        println(io, "$(round(Int,seat_cap_keys[i])),$(string(status_found))")
    end

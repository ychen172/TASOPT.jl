...
Also compute the R1 R2 R3 ranges requirements
"""
...
input_prefix    = "Opti_Jet_NoACT_to_Eth_for_Eth_" #Output name will be appended from this input name
Ranges_design   = [300.0,400.0] #collect(300:100:3000) #Must match
...
save_dir = joinpath(model_dir, input_prefix*"OffDes_")
mkpath(save_dir)
...
for (i,curRange) in enumerate(Ranges_design)
    ...
    #### run off-design
    save_name_cur = input_prefix*"OffDes_"*des_ran_str*"_"
